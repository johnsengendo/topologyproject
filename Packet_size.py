...
def create_linear_topology():
    ...
    with open('iperf_results_60_0.5_packet', 'a') as results_file:
        duration = 60
        interval = 0.5
        num_runs = 2  # Number of times to run iperf
        packet_sizes = [64, 128, 256, 512, 1024, 1470]  # Different packet sizes to test
        ...
        for i in range(num_runs):
            for packet_size in packet_sizes:
                ...
                result = net.get('h1').cmd(f'iperf -c {h2_ip} -i {interval} -t {duration} -b 10m -l {packet_size} -d')
                ...
                results_file.write(f"Test {i+1} (Packet size: {packet_size} bytes):\n{result}\n")
                results_file.write("-----\n")
