...
def create_linear_topology():
    ...
    # Introduce artificial delay and packet loss using NetEm
    net.get('h1').cmd('tc qdisc add dev h1-eth0 root netem delay 50ms loss 2%')
    net.get('h2').cmd('tc qdisc add dev h2-eth0 root netem delay 50ms loss 2%')

    # Running iperf multiple times between the hosts
    # Opening a file in append mode to write the results
    with open('iperf_results_60_0.5_packetloss', 'a') as results_file:
        duration = 60
        interval = 0.5
