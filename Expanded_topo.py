...
def run_parallel_iperf(net, packet_size, num_runs):
    ...
    with open('iperf_parallel_results.txt', 'a') as results_file:
        for i in range(num_runs):
            ...
            results_file.write(f"Run {i+1} - Client h1 to h3 with packet size {packet_size} bytes:\n{result_h1}\n-----\n")
            results_file.write(f"Run {i+1} - Client h2 to h4 with packet size {packet_size} bytes:\n{result_h2}\n-----\n")
...
if __name__ == "__main__":
    ...
    # Run the parallel Iperf tests with packet size 64 for 5 times
    run_parallel_iperf(net, 64, 5)
