...
def create_linear_topology():
    ...
    with open('iperf_results.txt', 'a') as results_file:
        for i in range(20):  # running the test 5 times
            ...
            # running the test for 1 second
            result = net.get('h1').cmd(f'iperf -c {h2_ip} -u -i 0.7 -b 10m')
            # result = net.get('h1').cmd(f'iperf -c {h2_ip} -i 0.7 -b 10m')
