    ...
    with open('iperf_results_60_0.5_mutipleiperf', 'a') as results_file:
        ...
        for i in range(num_runs):
            ...
            for j in range(num_connections):
                ...
                #thread = threading.Thread(target=run_iperf, args=(net.get('h1'), h1_ip, h2_ip, duration, interval, 5000 + j, results_file))
                thread = threading.Thread(target=run_iperf, args=(net.get('h1'), h2_ip, h2_ip, duration, interval, 5000 + j, results_file))
...
def run_iperf(h1, h2_ip, duration, interval, port, results_file):
    # Running iperf test from host 1 to host 2 using the IP address and print the results
    result = h1.cmd(f'iperf -c {h2_ip} -i {interval} -t {duration} -b 10m -p {port} -d')
