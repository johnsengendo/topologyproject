...
def run_iperf_flow(h1, h2_ip, server_port, duration, results_file):
    result_file = f"/tmp/iperf_flow_{server_port}.txt"
    h1.popen(f'iperf -c {h2_ip} -p {server_port} -i 0.5 -t {duration} -b 10m -d > {result_file}', shell=True)
    ...
    with open('iperf_results_multi_flows_updated', 'a') as results_file:
        ...
        for i, duration in enumerate(durations):
            for j in range(num_runs):
                ...
                for flow_id in range(num_flows):
                    server_port = 5000 + flow_id
                    thread = threading.Thread(target=run_iperf_flow, args=(net.get('h1'), h2_ip, server_port, duration, results_file))
                    thread.start()
